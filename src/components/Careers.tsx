...
import { ArrowLeft, MapPin, Clock, Users, Calendar, Send, CheckCircle, Building2, Code, Database, Globe, Brain, Settings, Smartphone, Eye, X } from 'lucide-react';
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-300">Join our global team of technology innovators</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-4xl mx-auto leading-relaxed">
            "Build with empathy, engineer with excellence." Join our global team and work on cutting-edge projects across 15+ countries.
          </p>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {jobsData.featuredJobs.map((job, index) => {
              ...
              return (
                <div
                  key={index}
                  className="group relative bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-3xl overflow-hidden hover:border-gray-600/50 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2"
                >
                  {/* Gradient Background */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${job.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-500 rounded-3xl`} />
                  
                  <div className="relative p-8">
                    <div className="flex items-start gap-6 mb-6">
                      <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${job.gradient} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                        <IconComponent size={24} className="text-white" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-3">
                          <h4 className="text-2xl font-bold text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-400 group-hover:to-purple-400 transition-all duration-300">
                            {job.title}
                          </h4>
                    ...
                    <div className="mb-6">
                      <div className="flex flex-wrap gap-2">
                        {job.skills.slice(0, 6).map((skill, skillIndex) => (
                          <span
                            key={skillIndex}
                            className="px-3 py-1 bg-gray-600/50 text-gray-300 text-sm rounded-full border border-gray-500/50"
                          >
                            {skill}
                          </span>
                        ))}
                        {job.skills.length > 6 && (
                          <span className="px-3 py-1 bg-gray-600/50 text-gray-400 text-sm rounded-full border border-gray-500/50">
                    ...
                    <div className="flex gap-4">
                      <Link
                        to={`/job/${job.id}`}
                        className="flex-1 bg-gray-600/50 backdrop-blur-sm border border-gray-500/50 text-white px-6 py-3 rounded-xl hover:bg-gray-500/50 hover:border-gray-400/50 transition-all duration-300 font-semibold text-center flex items-center justify-center gap-2"
                      >
                        <Eye size={16} />
                        View Details
                      </Link>
                      <button 
                        ...
                        className={`flex-1 bg-gradient-to-r ${job.gradient} text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all duration-300 transform hover:scale-105 font-semibold flex items-center justify-center gap-2`}
                      >
                        <Send size={16} />
                        Quick Apply
                      </button>
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {jobsData.allJobs.map((job, index) => {
              const IconComponent = getIconComponent(job.icon);
              return (
                <div
                  key={index}
                  className="group relative bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-3xl overflow-hidden hover:border-gray-600/50 transition-all duration-500 hover:shadow-xl hover:-translate-y-2"
                >
                  {/* Gradient Background */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${job.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-500 rounded-3xl`} />
